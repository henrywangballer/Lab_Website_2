...
function loadStatsPage(){
		...
		{//Outer for loop iterates over each row
			...
			for(col_counter = 1; col_counter < table.rows[row_counter].cells.length; col_counter++)
			{
				...
				if (col_counter == 4){
					...
					if (homeScore > awayScore){
						table.rows[row_counter].cells[col_counter].innerHTML = hteam;//Update the actual html of the cell
						wins++;
						
					}else{
						table.rows[row_counter].cells[col_counter].innerHTML = team;//Update the actual html of the cell
						lose++;
...
function loadPlayersPage(){
	//<a href="#" onclick="switchPlayers(1)">Player1</a><br>
	var player_selector = document.getElementById("player_selector");
	for (var i =0;i<players.length;i++){
		var archor = document.createElement('a');
		archor.href = '#';
		archor.setAttribute('onclick','switchPlayers('+i+')');
		archor.innerHTML = players[i].name;
		player_selector.appendChild(archor);
		player_selector.appendChild(document.createElement('br'));
	}
}
